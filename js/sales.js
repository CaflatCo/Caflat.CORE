...
function renderSalesTable() {
  ...
  ctl.innerHTML=`Showing ${Math.min(window.salesTableLimit||20,sales.length)} of ${sales.length}</span>
  ${(window.salesTableLimit||20) < sales.length ? '<button onclick="window.salesTableLimit=(window.salesTableLimit||20)+20;renderSalesTable()">Show More</button>' : (sales.length>20?'<button onclick="window.salesTableLimit=20;renderSalesTable()">Show Less</button>':'')}`;
